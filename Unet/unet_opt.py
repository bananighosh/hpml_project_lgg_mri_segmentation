...
## Generate a list in pandas format
"""
...
# Generate path address array of mask and img


"""## Encode label"""

class LabelProcessor:
    """Encoding the label image, generating an array where each pixel of channel 1 is a category"""

    def __init__(self):
        self.colormap = self.read_color_map()
        self.cm2lbl = self.encode_label_pix(self.colormap)
    
    # Tag encoding, returns the encoded tag of channel 1 eg: [0000000][0011000][0000000]
    def encode_label_img(self, img):
        data = np.array(img, dtype='int32')
        idx = (data[:, :, 0] * 256 + data[:, :, 1]) * 256 + data[:, :, 2]
        return np.array(self.cm2lbl[idx], dtype='int64')
    
    # Load color map, eg: 0th -> black, 1st -> white
    @staticmethod
    def read_color_map():  
        colormap = []
    ...
    # Label encoding, return hash table eg: cm2lbl[0] = 0, cm2lbl[(255*256+255)*256+256] = 1
    @staticmethod
...
class MRIDataset(Dataset):
    """Pass in the pandas format eg: img_path = pandas["image"]"""
    
    def __init__(self, img_path, label_path):
        # Read in the image and label paths, and pass in pandas format eg: img_path = pandas["image"]
    ...
    def __getitem__(self, index):
        ...
        # Read data from the file name (pictures and labels are image data in png format)
        img = cv2.imread(img)
    ...
    def img_transform(self, img, label):
        # Do some numerical processing on images and labels
        transform_img = transforms.Compose([transforms.ToTensor(),  # Convert tensor
